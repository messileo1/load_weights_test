...
class BasicConv(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, stride=1):
        ...
        self.bn = nn.BatchNorm2d(out_channels) #有BN的话bias设置为False
...
class YoloBody(nn.Module):
    def __init__(self, anchors_mask, num_classes, phi=0):
        ...
        self.phi            = phi #是否使用注意力机制，为0时不使用，默认是0，之后修改
        self.backbone       = darknet53_tiny() #不加载主干的预训练权重
        ...
        if 1 <= self.phi and self.phi <= 4:
            self.feat1_att      = attention_block[self.phi - 1](256) #实例化的时候只需要传入通道数即可
    ...
    def forward(self, x):
        ...
        #---------------------------------------------------#
        feat1, feat2 = self.backbone(x) # 传入输入x后自动调用CSPDarkent53_tiny里面的forward函数，返回两个特征层

        if 1 <= self.phi and self.phi <= 4:
            feat1 = self.feat1_att(feat1) # 注意力机制的使用
        ...
        return out0, out1 # 13,13,255  26,26,255
